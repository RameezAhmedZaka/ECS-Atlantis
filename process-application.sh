  ...
  if [[ -f "$d/main.tf" ]]; then
    ...
    case "$ENV" in
      "production")
        ...
        key="application/${WORKSPACE}/${ENV}/terraform1.tfstate"        # Relative to app directory
        ;;
      "staging")
        BACKEND_CONFIG="env/staging/stage.conf"    # Relative to app directory
        VAR_FILE="config/stage.tfvars"             # Relative to app directory
        key="application/${APP_NAME}/${WORKSPACE}/terraform1.tfstate"        # Relative to app directory
        ;;
    esac
    echo "Directory: $d"
