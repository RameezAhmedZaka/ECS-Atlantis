...
update_provider_tf() {
    ...
    if [ -f "$project_dir/providers.tf" ]; then
        echo "    Moving providers.tf to providers.tf.backup to avoid conflicts"
        mv "$project_dir/providers.tf" "$project_dir/providers.tf.backup"
    fi
    
    # Check if provider file exists
    if [ ! -f "$provider_file" ]; then
        # Create new provider.tf if it doesn't exist
        cat > "$provider_file" <<EOF
# This file is automatically generated by Atlantis
# It configures the AWS provider to assume the appropriate role
...
# AWS Provider configuration with assume_role
provider "aws" {
...
EOF
        echo "    Created $provider_file with role: $role_arn"
    else
        # Check if provider block already has assume_role
        if grep -q "assume_role.*{" "$provider_file"; then
            ...
            echo "    Updated existing assume_role in provider.tf with role: $role_arn"
        else
            # Add assume_role block to existing provider configuration
            # First, check if provider "aws" block exists
            if grep -q "provider[[:space:]]*\"aws\"" "$provider_file"; then
                # Add assume_role inside existing provider block
                local temp_file=$(mktemp)
                awk -v role="$role_arn" '
                /provider[[:space:]]*"aws"[[:space:]]*{/ {
                    print $0
                    print "  assume_role {"
                    print "    role_arn = \"" role "\""
                    print "  }"
                ...
                echo "    Added assume_role block to existing provider in $provider_file"
            else
                # No provider block found, append one
                cat >> "$provider_file" <<EOF
...
EOF
                echo "    Appended provider block with assume_role to $provider_file"
            fi
        fi
    fi
