...
#             else
#                 for env in $envs; do
#             in this the destroy is not working when someone run the command like
# atlantis plan -p appname -- -destroy 
# it should print the above msg but it is not printing why?        cat >> atlantis.yaml << PROJECT_EOF
#   - name: ${base_dir%/}-${app_name}-${env}
#     dir: $sub_dir
#     autoplan:
#       enabled: true
