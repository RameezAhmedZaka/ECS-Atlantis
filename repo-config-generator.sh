...
main_files_changed() {
    if [ -z "$CHANGED_FILES" ]; then
        return 0  # If we can't detect changes, assume main files changed (include all)
    fi
    echo "$CHANGED_FILES" | grep -q -E "(\.tf$|\.tfvars$)" | grep -v "/env/"
}
...
for base_dir in */; do
    ...
    for app_dir in "$base_dir"*/; do
        ...
        if is_terraform_project "$app_dir"; then
            ...
            # Check if main files changed (should trigger all environments for this app)
            main_changed=$(main_files_changed && echo "true" || echo "false")
            
            # Track if we found any environments with changes for this app
            found_envs=false
            ...
            for env in helia staging production; do
                ...
                env_changed=$(has_changes "$env_path" && echo "true" || echo "false")
                
                # Include this environment if:
                # 1. Main files changed (triggers all environments for this app), OR
                # 2. This specific environment directory changed
...
PROJECT_EOF
                    found_envs=true
                    echo "Including ${base_name}-${app_name}-${env} - main_changed: $main_changed, env_changed: $env_changed"
                else
                    echo "Skipping ${base_name}-${app_name}-${env} - no changes detected"
                fi
            done
            
            if [ "$found_envs" = "false" ] && [ "$main_changed" = "false" ]; then
                echo "No environments included for ${base_name}-${app_name} - no changes in main files or environments"
            fi
        fi
    done
done
