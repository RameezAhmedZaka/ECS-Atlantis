...
find_backend_config() {
    local project_dir="$1"
    local env="$2"
    local conf_dir="$project_dir/env/$env"
    if [ -d "$conf_dir" ]; then
        ls "$conf_dir"/*.conf 2>/dev/null | head -n1 || echo ""
    else
        echo ""
    fi
}
...
find_tfvars_file() {
    ...
    if [ -d "$config_dir" ]; then
        ls "$config_dir"/*.tfvars 2>/dev/null | grep -i "$env" | head -n1 || ls "$config_dir"/*.tfvars 2>/dev/null | head -n1 || echo ""
    else
        echo ""
    fi
}
...
get_project_name() {
    ...
    local parent=$(basename "$(dirname "$project_dir")")
    echo "${parent}-${base}"
}
...
for project_dir in "${projects[@]}"; do
    ...
    for env in $environments; do
        ...
        echo "      when_modified:" >> atlantis.yaml
        echo "        - \"$project_dir/*.tf\"" >> atlantis.yaml
        echo "        - \"$project_dir/config/*.tfvars\"" >> atlantis.yaml
        echo "        - \"$project_dir/env/*/*\"" >> atlantis.yaml
        echo "    terraform_version: v1.6.6" >> atlantis.yaml
        echo "    workflow: ${env}_workflow" >> atlantis.yaml
        echo "    apply_requirements:" >> atlantis.yaml
        echo "      - approved" >> atlantis.yaml
        echo "      - mergeable" >> atlantis.yaml
    done
done
...
for project_dir in "${projects[@]}"; do
    ...
    for env in $environments; do
        backend_config=$(find_backend_config "$project_dir" "$env")
        tfvars_file=$(find_tfvars_file "$project_dir" "$env")
        [ -n "$backend_config" ] && [ -n "$tfvars_file" ] || continue
