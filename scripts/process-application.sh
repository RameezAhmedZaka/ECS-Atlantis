...
echo "=== STARTING TERRAFORM PLAN for $ENV at $(date) ==="

# Parse arguments (destroy is disabled)
DESTROY_FLAG=false
APP_FILTER=""
...
echo "$dirs" | while IFS= read -r d; do
  if [ -f "$d/main.tf" ]; then
    ...
    if [ -n "$APP_FILTER" ] && [ "$APP_NAME" != "$APP_FILTER" ]; then
      echo "=== Skipping $APP_NAME planning (does not match filter: $APP_FILTER) ==="
      continue
    fi

    echo "=== Planning $APP_NAME ($ENV) ==="
    
    # Determine var file based on environment
    case "$ENV" in
      "production")
        VAR_FILE="config/production.tfvars"
        ;;
    ...
    PLAN_NAME="application_${APP_NAME}_${ENV}.tfplan"
    PLAN="/tmp/${PLAN_NAME}"
    echo "Step 1: Planning... Output: $PLAN"
    
    # Add destroy flag if needed (though it's disabled)
    DESTROY_ARG=""
    if [ "$DESTROY_FLAG" = "true" ]; then
      DESTROY_ARG="-destroy"
      echo "DESTROY MODE ENABLED"
    ...
    # Plan and capture output
    if timeout 300 terraform -chdir="$d" plan -input=false -lock-timeout=5m -var-file="$VAR_FILE" $DESTROY_ARG -out="$PLAN" 2>&1 | tee "$PLAN_OUTPUT"; then
...
echo "=== PLANNING COMPLETED for $ENV at $(date) ==="
echo "Applications with changes: $processed_count"
echo "Changed applications list:"
cat "$CHANGED_APPS_LIST" 2>/dev/null || echo "No applications with changes"
echo "Plan files created:"
cat "$PLANLIST" 2>/dev/null || echo "No plan files created"